...
func bpdLoadCgroups(src, dst, progType, where, callsMap, cgroup, bpfMount string, extraCFlags []string) error {
	...
	if _, err := cmd2.CombinedOutput(log, true); err != nil {
		if exitErr, ok := err2.(*stdlibExec.ExitError); ok {
			retCode = exitErr.ExitCode()
		}
	}

	cmd3 := exec.CommandContext(context.Background(), "cilium-map-migrate", "-e", outt, "-r", retCode)
	cmd.Env = bpf.Environment()
	if err := cmd3.CombinedOutput(log, true); err != nil {
		return err
	}
	...
	if err := cmd4.CombinedOutput(log, true); err != nil {
		return err
	}
